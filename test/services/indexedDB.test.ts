...
import { initDB, DB_NAME, DB_VERSION, SHOPS_STORE, LOCATIONS_STORE } from "../../src/services/indexedDB";
...
vi.mock('idb', async () => {
  ...
  describe('initDB', () => {
    ...
    beforeEach(() => {
      ...
      (openDB as unknown as jest.Mock).mockImplementation((_: string, __: number, options: { upgrade: (db: IDBDatabase) => void }) => {
    ...
    it('should call openDB with correct parameters', async () => {
      await initDB();
      expect(openDB).toHaveBeenCalledWith(DB_NAME, DB_VERSION, expect.any(Object));
    });
    ...
    it('should create SHOPS_STORE if it does not exist', async () => {
      ...
      await initDB();
      expect(mockDB.createObjectStore).toHaveBeenCalledWith(SHOPS_STORE, { keyPath: 'id' });
    });
    ...
    it('should not create SHOPS_STORE if it already exists', async () => {
      ...
      await initDB();
      expect(mockDB.createObjectStore).not.toHaveBeenCalledWith(SHOPS_STORE, expect.anything());
    });
    ...
    it('should create LOCATIONS_STORE if it does not exist', async () => {
      (mockDB.objectStoreNames.contains as jest.Mock).mockImplementation((store) => store !== LOCATIONS_STORE);
      await initDB();
      expect(mockDB.createObjectStore).toHaveBeenCalledWith(LOCATIONS_STORE, { keyPath: 'id' });
    });
    ...
    it('should not create LOCATIONS_STORE if it already exists', async () => {
      (mockDB.objectStoreNames.contains as jest.Mock).mockReturnValue(true);
      await initDB();
      expect(mockDB.createObjectStore).not.toHaveBeenCalledWith(LOCATIONS_STORE, expect.anything());
    });
  });
